...
14.中文显示乱码
    终端只支持ascii码显示，不支持unicode码

15.进程通信遇到问题
    15.1 重入内核
         当发生中断时，处于内核态，在此情况下调用了处于内核态时，调用了send_receive(处于ring3),因为该函数调用了sendreceive，该函数内部出发了一个软中断，在次进入内核，所以发生重入内核这个错误。
         总结：在不同等级（ring0-3）间调用函数的时候要慎重，即使是<ring0>函数调用<ring3>函数也同样如此。上述问题就是ring0调用ring3引发的。
    15.2 中断消息的处理
         有问题的处理方式：中断消息的收发同一般消息的首发很不一样。中断的发送发送处于内核态，没有pid，也就没有对应的进程，就不能向一般消息那样处理。还有就是中断消息可能先发送，再接受，这主要是进程调度、内核态中断处理程序的运行问题。如，硬盘操作，当读取硬盘（1），给硬盘发送命令（2）后，等待硬盘中断（3），但是在给硬盘发送命令后，如果命令在很短的时间内完成，那么内核对应中断处理程序就会发送中断消息，且是在（3）是前就已经发送，这就导致了一个很令人迷惑的问题。所以通过进程的n_waiting_int来记录进程是否在等待中断，如果为0，则没有等待；否则为处于等待中断、阻塞状态
         正确处理方式：在进行正确处理解决方案前，要了解为什么要写进程间通信模块，而中断有时运行在什么状态<ring?>?由于<ring2/3>进程不能访问对方的内存空间，所以要通过软中段进入内核态<ring0>(<ring0>内核态才可以写任何内存空间),实现进程间的通信，这也正式消息通信模块完成的功能。由于中断是发生在内核态，所以中断消息的处理和其他一般消息的处理有本质的区别。所以处理方法为：
             a>中断发生时，对应进程正在等待该中断消息，则该直接告诉进程，唤醒进程
             b>如果对应进程没有等待该中断消息，则设置该进程的已经接受到一个中断消息，在该进程通过正常的消息通信模块进行消息接受处理的时候，如果是要接受中断消息，则可以立刻唤醒进程，告知其已经接受到了一个对应的中断消息。
         处理本问题的误区：试图通过前面有问题的方法，统一处理所有的进程通信，结果导致关系混乱，主要问题在于：如果是中断消息时，其没有特定发送进程（没有pid,他是在直接运行于内核态的）
